...
## Gambletron
Gambletron.wp <- read.csv("~/Dropbox/BlogPosts/BlogPosts/super_bowl_li.csv")

## ESPN
espn.wp <- read.csv("~/Dropbox/BlogPosts/BlogPosts/super_bowl_li.csv")


## Halftime is 8:03 to 8:35, so drop those
## Overtime starts at 10:14, so drop those
...
ann_text <- data.frame(x = c(450, 450, 450, 450, 450), 
                       y1 = c(0.90, 0.85, 0.8, 0.75, 0.7), 
                       y2 = c(900, 850, 800, 750, 700),
                       lab = c("Pro Football Reference", "nflscrapr", 
                               "Lock & Nettleton", "Gambletron", "phdFootball"), 
                       int = c(min(pfr.wp$Pats.wp), min(cmu.wp$Pats.wp), min(LockNett.Pats$Pats.wp), 
                               min(Gambletron.wp$Pats.wp), min(phd.wp$Pats.wp)), 
...
ggplot(LockNett.Pats, aes(3600-seconds, Pats.wp)) + geom_path(colour = cols[3]) + 
...
el.all <- Reduce(intersect, list(el1$seconds, el2$seconds, el3$seconds, el4$seconds))

elig.1 <- cmu.wp$seconds[cmu.wp$seconds %in% elig.plays$seconds]
...
winprob.Pats.all %>% mutate(agree.all = (Lock_Nettleton > 0) & (Pro_Football_Reference) > 0 & (nflscrapR > 0) & (PhDfootball > 0) | 
                              (Lock_Nettleton < 0) & (Pro_Football_Reference) < 0 & (nflscrapR < 0) & (PhDfootball < 0) ) %>%
  count(agree.all)



round(filter(winprob.Pats.all, PhDfootball < 0.01, Pro_Football_Reference > 0.05), 2)
...
ggpairs(winprob.Pats.all[,2:ncol(winprob.Pats.all)], lower = list(continuous = my_fn), diag = list(continuous = my_fn2)) + 
  labs(title = "New England's win probability added per play in Super Bowl 51", 
       subtitle = "Rushes and passes only (either team on offense)") 
